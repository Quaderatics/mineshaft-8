...
execute if entity @a[tag=debug] run execute if entity @s[tag=new_game] run say This is a new game
...
# Normal level
execute if entity @a[tag=debug] if score #next_is_anomalous value matches 0 run say Selected normal level
...
# Anomalous level
execute if entity @a[tag=debug] run say Selected anomalous level
...
scoreboard players operation #index value %= #num_available_anomalies value
